...
uint32_t sys_mkdir(char * file_name)
{
    invoke_syscall(SYSCALL_MKDIR, (int) file_name, IGNORE, IGNORE);
}

uint32_t sys_cd(char * file_name)
{
    invoke_syscall(SYSCALL_CD, (int) file_name, IGNORE, IGNORE);
}

void sys_ls(char * file_name, int * location)
{
    invoke_syscall(SYSCALL_LS, (int) file_name, (int) location, IGNORE);
}

void sys_statfs(int * location)
{
    invoke_syscall(SYSCALL_STATFS, (int) location, IGNORE, IGNORE);
}

uint32_t sys_rmdir(char * file_name)
{
    invoke_syscall(SYSCALL_RMDIR, (int) file_name, IGNORE, IGNORE);
}

int sys_touch(char *file_name)
{
    invoke_syscall(SYSCALL_TOUCH, (int) file_name, IGNORE, IGNORE);
}

int sys_cat(char *file_name, int *location)
{
    invoke_syscall(SYSCALL_CAT, (int) file_name, (int) location, IGNORE);
}

int sys_fopen(char *file_name, int access)
{
    invoke_syscall(SYSCALL_FOPEN, (int) file_name, access, IGNORE);
}
...
void sys_launch(char *file_name)
{
    invoke_syscall(SYSCALL_LAUNCH, (int) file_name, IGNORE, IGNORE);
}
