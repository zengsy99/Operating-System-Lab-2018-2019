...
void do_launch(char *file_name)
{
    ...
    for (i = 0; i < current_dir_depth; i++)
    {
        ...
        if(j == inode_buffer.dentry_num)
        {
            printk("[Error] Path Error in do_launch_1!");
            return;
        }
    }
    ...
    if(j == inode_buffer.dentry_num)
    {
        printk("[Error] Path Error in do_launch_2!");
        return; // Path Error in the middle path
    }
    sdread(&inode_buffer, superblock.inode_start + current_ino * INODE_SIZE, sizeof(inode_t));
    sdread(block_buf, superblock.data_start + inode_buffer.direct_block[0] * BLOCK_SIZE, BLOCK_SIZE);
    ...
    Elf32_Ehdr Ehdr;
    Elf32_Phdr Phdr;
    memcpy(&Ehdr, block_buf, sizeof(Elf32_Ehdr));
    uint32_t e_phoff = (uint32_t ) Ehdr.e_phoff;
    memcpy(&Phdr, block_buf + e_phoff, sizeof(Elf32_Phdr));

    // Set TLB
    uint32_t vaddr = 0x2000;
    uint32_t VPN = (vaddr & 0xffffe000) >> 12;
    i = handle_page_fault(VPN);

    set_EntryHi(0x00002002);
    set_cp0_Index(16);
    set_EntryLo0(PTE[i  ].PFN << 6 | 0x16);
    set_EntryLo1(PTE[i+1].PFN << 6 | 0x16);
    set_PageMask();

    // Copy the code to paddr
    uint32_t p_filesz = Phdr.p_filesz;
    uint32_t p_offset = Phdr.p_offset;
    memcpy( (PTE[i].PFN << 12) + 0xa0000000, block_buf + p_offset, p_filesz);
    ...
    pcb->kernel_context.regs[29]    = STACK_TOP_K - 2*STACK_SIZE; 
    pcb->user_context.regs[29]      = STACK_TOP_K - 400;//STACK_TOP_U;

    pcb->user_context.cp0_status    = 0x1000ff00;
    pcb->user_context.hi            = 0;
    pcb->user_context.lo            = 0;
    pcb->user_context.cp0_badvaddr  = 0;
    pcb->user_context.cp0_cause     = 0;
    pcb->user_context.cp0_epc       = vaddr;
    pcb->user_context.pc            = 0;
    pcb->user_stack_top             = STACK_TOP_U;

    pcb->kernel_context.cp0_status  = 0x1000ff00;
    ...
    pcb->kernel_stack_top           = STACK_TOP_K - 2*STACK_SIZE;

    pcb->pid  = 2;
    pcb->type = USER_PROCESS;
