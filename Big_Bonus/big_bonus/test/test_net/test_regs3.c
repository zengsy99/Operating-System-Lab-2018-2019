...
void phy_regs_task1()
{
    ...
    register_irq_handler(LS1C_MAC_IRQ, (int) irq_mac);
...
void phy_regs_task2()
{
    ...
    int fd = sys_fopen("hello", O_RDWR);

    for(i=0; i<PNUM; i++)
        if(Rx_buffer[i][0]==0x84541b10 && Rx_buffer[i][1]==0xe290d683 && Rx_buffer[i][13]==0x0000000)
        {
            sys_fwrite(fd, (char*)&Rx_buffer[i][14], 900);
            break;
        }

    for(i=0; i<PNUM; i++)
        if(Rx_buffer[i][0]==0x84541b10 && Rx_buffer[i][1]==0xe290d683 && Rx_buffer[i][13]==0x1000000)
        {
            sys_fwrite(fd, (char*)&Rx_buffer[i][14], 910);
            break;
        }

    for(i=0; i<PNUM; i++)
        if(Rx_buffer[i][0]==0x84541b10 && Rx_buffer[i][1]==0xe290d683 && Rx_buffer[i][13]==0x2000000)
        {
            sys_fwrite(fd, (char*)&Rx_buffer[i][14], 910);
            break;
        }
