...
int main(int argc, char *argv[])
{
    ...
    if (mode == LISTENING_MODE) {
	    ...
	    FILE *hello = fopen("hello","rb+");
	    fseek(hello,0L,SEEK_END);
	    int flen=ftell(hello);
	    printf("flen = %d\n",flen);
	    char *p = (char *) malloc(flen+1);
			
	    fseek(hello, 0L, SEEK_SET);
	    ttt = fread(p,sizeof(char), flen, hello);
	    Elf32_Ehdr eh;
	    Elf32_Phdr ph;
	    ...
	    uint32_t p_filesz = ph.p_filesz;
            ...
            printf("e_phoff=0x%x, filesz=0x%x, offset=0x%x \n", e_phoff, p_filesz, p_offset);
	    uint32_t *tp = (uint32_t *) (p + p_offset);

	    int num=25; int count = 0;
	    while(num--)
            {
	        fseek(hello, 0L, SEEK_SET);

	        ttt = fread(p,sizeof(char), 900, hello);
	        send_packet(p, 900);
	        printf("package: %d\n", count++);
	        time_wait(5);

	        ttt = fread(p,sizeof(char), 910, hello);
	        send_packet(p, 910);
	        printf("package: %d\n", count++);
	        time_wait(5);

	        ttt = fread(p,sizeof(char), flen-1810, hello);
