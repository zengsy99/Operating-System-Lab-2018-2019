...
void check_recv(mac_t *test_mac)
{
    ...
    while(received_num < PNUM)
    {
        if(!(r_desc[received_num].tdes0  >> 31))
        {
            ...
            if(recv_addr[0]!= 0x5e0001 && recv_addr[0]!= 0x7f5e0001 && recv_addr[0]!= 0xffffffff && recv_addr[0]!=0x3333 && received_num%8 == 0) // The last requirement is to save time.....otherwise it will take lots of time to print packages
                for(j = 0; j < PSIZE; j++)
...
int register_irq_handler(uint32_t data, uint32_t irq_mac)
{
...
uint32_t do_net_recv(uint32_t rd, uint32_t rd_phy, uint32_t daddr)
{
    ...
    reg_write_32(DMA_BASE_ADDR + DmaRxBaseAddr, rd_phy);                        // DMA #3  Physical head address of DMA
    ...
    for(i = 0; i < PNUM; i++)
        r_desc[i].tdes0 = DescOwnByDma;                    // enable recv
...
void do_net_send(uint32_t td, uint32_t td_phy)
{
    ...
    reg_write_32(DMA_BASE_ADDR + DmaTxBaseAddr, td_phy);                        // DMA #4  Physical head address of DMA 
    reg_write_32(GMAC_BASE_ADDR, reg_read_32(GMAC_BASE_ADDR) | GmacTxEnable);   // enable send

    reg_write_32(DMA_BASE_ADDR + 0x18, reg_read_32(DMA_BASE_ADDR + 0x18) | 0x02202000); //0x02202002); // start tx, rx // DMA #6
    reg_write_32(DMA_BASE_ADDR + 0x1c, 0x10001 | (1 << 6));                                                            // DMA #7
